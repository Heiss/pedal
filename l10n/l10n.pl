	...
	foreach my $dir ( @dirs ){
		...
		foreach my $file ( @totranslate ){
			...
			if( $file =~ /\.js$/ ){
				...
				$keywords = '--keyword=gettext --keyword=n:1,2';
			}
			my $language = ( $file =~ /\.js$/ ? 'Javascript' : 'PHP');
			my $joinexisting = ( -e $output ? '--join-existing' : '');
			print "    Reading $file\n";
			`xgettext --output="$output" $joinexisting $keywords --language=$language "$file" --add-comments=TRANSLATORS --from-code=UTF-8 --package-version="0.1" --package-name="Pedal" --msgid-bugs-address="peter.heiss@uni-muenster.de"`;
		}
		chdir( $whereami );
	}
}
